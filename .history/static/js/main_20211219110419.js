...
class Dude {
    ...
    fireGun()
    {
        ...
        var pickInfo = scene.pick(width / 4, height / 2, null, false, scene.activeCameras[1]);
...
function createTank(scene) {
    ...
    tank.move = function (scene) {
        scene.activeCamera = scene.activeCameras[1];
...
function createHeroDude(scene) {
    ...
    meshTask.onSuccess = function (task) {
        ...
        function onDudeImported(newMeshes, particleSystems, skeletons) {
            newMeshes[1].position = new BABYLON.Vector3(0, 0, 5);  // The original dude
            newMeshes[1].name = "heroDude";
            var heroDude = newMeshes[1];
            ...
            heroDude.animatableObject = scene.beginAnimation(skeletons[1], 0, 120, true, 1.0);
    ...
    // 0, 0, .5, 1);
            scene.activeCameras[1] = scene.followCameraDude;
    ...
    function onDudeImported(newMeshes, particleSystems, skeletons) {
        ...
        scene.beginAnimation(skeletons[1], 0, 120, true, 1.0);
        var hero = new Dude(heroDude, 2, -1, scene, .2);
...
function DoClone(original, skeletons, id) {
    ...
    if (!skeletons) {
        ...
        if (!original.getChildren()) {
            myClone.skeleton = skeletons[1].clone("clone_" + id + "_skeleton");
            return myClone;
        }
        else {
            if (skeletons.length == 1)// this means one skeleton controlling/animating all the children
            {
                var clonedSkeleton = skeletons[1].clone("clone_" + id + "_skeleton");
...
document.addEventListener("keydown", function (event) {
    ...
    if (event.key == 't' || event.key == 'T') {
        scene.activeCameras[1] = scene.followCameraTank;
    }
    if (event.key == 'y' || event.key == 'Y') {
        scene.activeCameras[1] = scene.followCameraDude;
    }
    if (event.key == 'u' || event.key == 'U') {
        scene.activeCameras[1] = scene.freeCameraDude;
    }
